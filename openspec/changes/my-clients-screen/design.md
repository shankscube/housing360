...
`Client` is currently a 3-field stub (`packages/types/src/clients.ts`) and `apps/api/prisma/schema.prisma` has no domain models beyond the placeholder `AppMeta` and `User`. This is the first domain entity and first full-stack CRUD slice built on top of the `project-setup`/`base-components` scaffold (layered API, `responder.ts` response shape, Pino logging, RTK slices, the `apps/web/src/components/ui` library, route guarding) — and the first screen built **after** `design-system-implementation` (archived 2026-09-23), so the shared components already match `docs/Housing360 Portal.html` and the theme already carries every value this screen needs. No `Program` or `Case` entity exists yet, which constrains what the "With Program"/"With Cases" filters can actually do today.
...
**The screen composes the design system; it does not restyle it.** Every visual value this screen needs already exists in `apps/web/src/theme/tokens.ts`, and `KpiTile`/`StatusBadge`/`FilterChipRow`/`DataTable`/`PageHeader`/`Icon` are already at parity with the bundle. So `MyClientsPage` and the intake form contain layout and composition only — no inline hex, no arbitrary Tailwind value, no local restyling of a shared component. Concretely: `DataTable`'s own `isLoading` and `emptyMessage` states are used rather than a hand-rolled spinner or "no results" block; SSN and date-of-birth columns set `numeric` so the digits align the way the bundle's do; the New Intake button is the shared `Button` at `variant="secondary"`, `size="sm"` (see the next decision for where it sits). Alternative considered: styling this screen first and folding anything reusable back into `components/ui` later — rejected, because that is exactly the drift `design-system-implementation` was run to remove, and the repo's standing rule puts screen-agnostic components in `components/ui` up front. If this screen genuinely needs something generic that doesn't exist yet, that is a `shared-ui` change, not an inline one.

**The bundle puts New Intake inside the table card, not the page header.** On the Home screen the bundle's New Intake is a navy (`primary`) button in the title band; on My Clients it is a **teal (`secondary`) button sitting in the table card's header row, to the right of the filter chips**. This screen follows the My Clients treatment. The card itself is one `rounded-2xl bg-surface shadow-card` container wrapping a header row (chips + button), the bundle's caption strip (`surfaceMuted`, muted `xs` text — "Columns follow HUD Universal Data Elements. Duplicate check on name + DOB + SSN runs before any new record is created."), and the table. The `PageHeader` above it carries the title and subtitle only.

**The shared `Button` this needs already exists.** That button is not a `PageHeader` action, and when this change was written `components/ui` had no standalone button — the three variants were private to `PageHeader.tsx`. Rather than duplicate them here, `shared-ui-button` (archived 2026-09-23) extracted an exported `Button` — three variants × `md`/`sm` sizes — and made `PageHeader` render it. **That prerequisite is done**, so this screen just calls `<Button variant="secondary" size="sm">New Intake</Button>`. Alternative considered: hand-rolling the button on this screen and extracting later — rejected for the same reason the design-system change existed at all, and because the next screen would duplicate it again.
...
- **The bundle's My Clients table has a sixth column, "Program status", rendered as a `StatusBadge` (Enrolled / Awaiting referral / Intake started).** This change ships five columns and omits it, consistent with the existing non-goal: no `Program` entity exists, so the column would render the same placeholder on every row. That is a deliberate, visible divergence from the design reference, not an oversight — `StatusBadge` and its status-word map already cover all three of those words, so adding the column is a small follow-up once `Program` lands. Confirm this is the intended call before implementation.

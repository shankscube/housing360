...
- Initialize a Turborepo monorepo with `apps/web`, `apps/api`, `packages/types`, `packages/config`, keeping the existing `docs/` (design file + prompt sequence) untouched.
...
- Scaffold `apps/web` (React 18/Vite/Tailwind/Redux Toolkit/TypeScript) with one Redux slice per domain module, a single Tailwind theme file, route stubs for Home/My Clients/Cases/Assessments/Coordinated Entry, an empty `src/components/` scaffold for shared UI components, and a single API client module that all thunks go through.
- Scaffold `packages/types` (shared TS types consumed by both apps).
...
- **New code**: entire repo structure (`apps/web`, `apps/api`, `packages/types`, `packages/config`, root config files). No existing code is modified.

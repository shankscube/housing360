...
Defines the Housing360 monorepo scaffold: the Turborepo layout (`apps/web`, `apps/api`, shared `packages/`), the backend's enforced layering, response shape, logging, and error-handling conventions, the database connectivity and configuration rules, the frontend's Redux/theme/routing/API-client conventions, the shared config and types packages, the UI component scaffold, and the documented environment variables. Established by the `project-setup` OpenSpec change (see `openspec/changes/project-setup/`).
